...
服务器重启后发现 apache 无法运行了，提示：

    H00526: Syntax error on line 14 of /usr/local/apache-2.4/conf/mod_security.conf:
    ModSecurity: Failed to open the audit log file: /tmp/modsecurity/modsecurity.log

查明由于重启后/tmp 目录被删除，因此`SecDebugLog`和`SecAuditLog`的配置失效，无法在/tmp 下找到目录，因此修改后两行配置为：

    SecDebugLog /tmp/modsec_debug.log # 输出调试日志
    SecAuditLog /tmp/modsecurity.log # 记录单条日志 多文件会记录简要信息帮助索引到www目录下的日志文件

而`SecAuditLogStorageDir`设置的目录会自动创建，无需手动创建和修改所有者。

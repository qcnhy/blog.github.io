...
经过努力还是出现频繁断连的情况，WIFI 功能也不太稳定，原因不详，将 WIFI 关闭后断连情况大幅改善，偶尔仍会出现模块断连的情况，疑似 USB 接口接触不良或者供电不足的情况，几秒后自动重连，无法解决，只能这样了。

## MBIM 模式联网

[官方文档](https://openwrt.org/docs/guide-user/network/wan/wwan/ltedongle)与 QMI 一起的
通过安装 istoreOS 系统，发现有多种拨号方式，而在那个系统中 qmi 方式无法正确识别，遂使用 MBIM 模式进行尝试
需要在模组中调整模式
![](/img/2025-10-08-22-06-13.png)

        opkg install umbim kmod-usb-net-cdc-mbim // 安装驱动
        AT+QCFG="usbnet"     // 查询当前模式
        AT+QCFG="usbnet",2   // 设置为 MBIM 模式
        mmcli -m "$(mmcli -L | grep -o '/Modem/[0-9]\+' | grep -o '[0-9]\+' | head -n 1)" --command='AT+QCFG="usbnet"' //使用mmcli执行AT命令，需要开启debug
        AT+CFUN=1,1  // 重启模块

使用 modemmanager 进行连接，出现错误

        Wed Oct  8 22:02:46 2025 kern.crit kernel: [   70.048866] cdc_mbim 2-1:1.4 wwan0: NETDEV WATCHDOG: CPU: 3: transmit queue 0 timed out 5570 ms

        opkg install luci-proto-mbim
        reboot

使用自带的连接工具进行连接，连接成功，看来使用 modemmanager 进行连接，无法连接上。
![](/img/2025-10-08-22-36-01.png)
断连问题仍存在。

## ECM 模式联网

[官方文档](https://openwrt.org/docs/guide-user/network/wan/wwan/ethernetoverusb_cdc)
安装 kmod-usb-net-cdc-ether 驱动

        opkg install kmod-usb-net-cdc-ether
        mmcli -m "$(mmcli -L | grep -o '/Modem/[0-9]\+' | grep -o '[0-9]\+' | head -n 1)" --command='AT+QCFG="usbnet",1' //使用mmcli执行AT命令切换ECM模式，需要开启debug
        AT+CFUN=1,1 //重启模块

IPV6 时而可能获取不到  
经过切换几种模式的测试，使用自带的不论是 qmi 还是其他的，自带的连接对于自动重连等能力更为可靠些。
